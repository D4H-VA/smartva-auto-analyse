...
Version: 2.15 - Philippines - English
...
if(!(Test-Path $resultsfile)){
    ...
    "VA_ID," + "Age," + "Sex," + "CoD1," + "Lh1," + "CoD2," + "Lh2," + "CoD3," + "Lh3," + "All_Symptoms," + "Date_Entered," + "PCoD," | Add-Content $resultsfile
}
...
if($config.Aggregate -ne '1'){
    ...
    if(($bprocPull.ExitCode -eq 1) -or ($bprocExport.ExitCode -eq 1) ){
        ...
        if($errors -eq $null){
            ...
            $ObjLabel.Text = "ODK Briefcase Export completed ... 42%"
            $PB.Value = 42
            $ObjForm.Refresh()
            Start-Sleep -Milliseconds 150
                            ...
                            if($archiveResults -ne $null){
                                ...
                                if($x -eq 0){
                                    if($displayResults -eq 1){
                                        do{
                                            $PhdUCoD = [Microsoft.VisualBasic.Interaction]::InputBox("$results`n`nPlease enter the cause of death on the death certificate:", "Physician Cause of Death", "")
                                        }
                                        until($PhdUCoD -ne "")
                                    }
                                    try{
                                        $sid + "," + $age + "," + $gender + "," + $cod1 + "," + $lh1 + "," + $cod2 + "," + $lh2 + "," + $cod3 + "," + $lh3 + "," + $as + "," + $datetime + "," + $PhdUCoD + "," | Add-Content -Encoding UTF8 $resultsfile -ErrorAction Stop
