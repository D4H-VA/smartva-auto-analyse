...
if(!(Test-Path $resultsfile)){
    ...
    "VA_ID," + "Age," + "Sex," + "CoD1," + "Lh1," + "CoD2," + "Lh2," + "CoD3," + "Lh3," + "All_Symptoms," + "PCoD," + "Date_Entered" | Add-Content $resultsfile
}
...
if($config.Aggregate -ne '1'){
    ...
    if($bproc.ExitCode -eq 1){
        ...
        if($errors -eq $null){
	        ...
	        $ObjForm.Refresh()
            Start-Sleep -Milliseconds 150
            $tproc = Start-Process -FilePath SmartVA-Analyze-cli.exe -ArgumentList "--country $tariffCountry --hiv $tariffHIV --malaria $tariffMalaria --hce $tariffHCE --freetext $tariffFreetext --figures $tariffFigures `"$tariffInput`" $tariffOutput $language" -NoNewWindow -PassThru -RedirectStandardError $logfiledir\TariffOutput.txt 
            $tproc
                ...
                if($tariffpredictions -ne $null){
                    ...
                    ForEach ($prediction in $tariffpredictions){
                        ...
                        ForEach ($VA in $VAs){
                            ...
                            $results += "VA ID:  " + $sid + "`nAge:  " + $age + "`nSex:  " + $gender + "`n`nCause of Death 1:  " + $cod1 + "`nLikelihood:  " + $lh1 + "`n`nCause of Death 2:  " + $cod2 + "`nLikelihood:  " + $lh2 + "`n`nCause of Death 3:  " + $cod3 + "`nLikelihood:  " + $lh3 + "`n`n" + "All Symptoms:`n" + $as_repl
                    ...
                    if($x -gt 0){
                        (new-object -ComObject wscript.shell).Popup("No results to display.  All results are already in the archive results file.",0,"SmartVA Results")
                    }
                }else{
