...
Add-Type -assembly System.Windows.Forms
    ...
    $ObjForm.Text = "SmartVA-Auto-Analyse Entradas de procesamiento..."
    $ObjForm.Height = 100
    $ObjForm.Width = 500
...
Get-Content $installPath\config.txt | foreach-object -begin {$config=@{}} -process { $k = [regex]::split($_,'='); if(($k[0].CompareTo("") -ne 0) -and ($k[0].StartsWith("[") -ne $True)) { $config.Add($k[0], $k[1]) } }
...
if(!(Test-Path $resultsfile)){
    ...
    "VA_ID," + "Identificación nacional," + "Tipo de identificación," + "el apellido," + "segundo apellido," + "el primer nombre," + "otro nombre," + "fecha de la muerte," + "Años," + "Sexo," + "CoD1," + "Lh1," + "CoD2," + "Lh2," + "CoD3," + "Lh3," + "Todos_los_síntomas," + "Fecha ingresada," + "PCoD," | Add-Content $resultsfile
}
...
if($config.Aggregate -ne '1'){
    ...
    $ObjLabel.Text = "ODK Briefcase Conversión comenzada ... 22%"
    $PB.Value = 22
    $ObjForm.Refresh()
    ...
    Do {
	    ...
	    $ProcessesFound = Get-Process | ? {$ProcessList -contains $_.Name} | Select-Object -ExpandProperty Name
	        If ($ProcessesFound) {
                ...
                $counter++
                $ObjLabel.Text = "ODK Briefcase Conversión comenzada ... $Percentage% "
                ...
                if($config.Aggregate -ne "1"){
                    ...
                    if($archiveFormFile.Fullname -ne $null){
                        ...
                        if(($collectFormFile.LastWriteTime.Date -gt $archiveFormFile.LastWriteTime.Date) -or ($collectFormFile.LastWriteTime.Date -eq $archiveFormFile.LastWriteTime.Date -and $collectFormFile.LastWriteTime.TimeofDay -gt $archiveFormFile.LastWriteTime.TimeofDay)){
                            ...
                            try{
                                ...
                                (new-object -ComObject wscript.shell).Popup("Se ha encontrado un problema al copiar al archivo. Por favor mira $($logfile) para informacion adicional",0,"Error!", 48+0+4096)                       
                    ...
                    if($archiveInstanceFiles -ne $null){
                        ...
                        if($errorcount -gt 0){
                            ...
                            (new-object -ComObject wscript.shell).Popup("Se ha encontrado un problema al copiar al archivo.`n`nReview and correct the error then re-run analysis to ensure Archive data is updated BEFORE removing VA's from the Tablet.`n`nFor error details see $($logfile).",0,"Error!", 48+0+4096)
                        }
                    }else{
                        try{
                            ForEach($collectInstanceFile in $collectInstanceFiles){
                ...
                if($tariff_liklihood_files -ne $null){
                    ...
                    ForEach ($liklihood_file in $tariff_liklihood_files){
                        ...
                        ForEach ($VA in $VAs){
                            ...
                            ForEach ($line in $raw_data){
                                if($sid -eq $line.'Generalmodule-sid'){
                                    $NatID = $line.'general5-gen_5_2b0_es-gen_5_2b3_es'
                                    $Surname = $line.'general5-name-gen_5_0a'
                                    $Second_Surname = $line.'general5-name-gen_5_0a_es1'
                                    $First_Name = $line.'general5-name-gen_5_0'
                                    $Other_Name = $line.'general5-name-gen_5_0c'
                                    $Death_Date = $line.'deathDate-gen_5_3d'
                                    $ID_Type_No = $line.'general5-gen_5_2b0_es-gen_5_2b2_es'
                                    
                                    switch($ID_Type_No){
                                        1 {$ID_Type = "Registro Civil" ; break}
                                        2 {$ID_Type = "Tarjeta de Identidad" ; break}
                                        3 {$ID_Type = 'Cédula de Ciudadanía' ; break}
                                        4 {$ID_Type = 'Cédula de Extranjería' ; break}
                                        5 {$ID_Type = "Pasaporte" ; break}
                                        6 {$ID_Type = "Otro" ; break}
                                        7 {$ID_Type = "Dont Know" ; break}
                                        8 {$ID_Type = "None" ; break}
                                        9 {$ID_Type = "Tarjeta de ciudadanía de la madre" ; break}
                                        10 {$ID_Type = 'Cédula Nacional de identidad de la madre' ; break}
                            ...
                            $results += "VA ID:  " + $sid + "`nIdentificación nacional:  " + $NatID + "`nTipo de identificación:  " + $ID_Type + "`nel apellido:  " + $Surname + "`nsegundo apellido:  " + $Second_Surname + "`nel primer nombre:  " + $First_Name + "`notro nombre:  " + $Other_Name + "`nfecha de la muerte:  " + $Death_Date + "`nAge:  " + $age + "`nSex:  " + $gender + "`n`nCausa de la muerte 1:  " + $cod1 + "`nProbabilidad:  " + $lh1 + "`n`nCausa de la muerte 2:  " + $cod2 + "`nProbabilidad:  " + $lh2 + "`n`nCausa de la muerte 3:  " + $cod3 + "`nProbabilidad:  " + $lh3 + "`n`n" + "Todos los síntomas:`n" + $as_repl
                        ...
                        if($archiveResultErrorCount -gt 0){
                            (new-object -ComObject wscript.shell).Popup("Could not write results to Archive file $resultsfile due to: `n`n$archiveErrorMsg`n`n Si este archivo está abierto, ciérrelo y repita el análisis para capturar los resultados en el archivoSi este archivo está abierto, ciérrelo y repita el análisis para capturar los resultados en el archivo", 0,"Error", 48+0+4096)
                    ...
                    if($x -gt 0){
                        (new-object -ComObject wscript.shell).Popup("No hay resultados para mostrar. Todos los resultados ya están en el archivo de resultados del archivo.",0,"SmartVA-Auto-Analyse Results", 64+0+4096)
                    }elseif($displayResults -eq 0){
                        (new-object -ComObject wscript.shell).Popup("$resultcount resultado (s) añadido al archivo",0,"SmartVA-Auto-Analyse Results", 64+0+4096)
                    }
                }else{
                    (new-object -ComObject wscript.shell).Popup("No hay resultados para mostrar. Esto es normal si el VA procesado no tenía información sobre la edad, o no se proporcionó el consentimiento.", 0,"SmartVA-Auto-Analyse Results", 64+0+4096)
                }
            }
        } 
    }