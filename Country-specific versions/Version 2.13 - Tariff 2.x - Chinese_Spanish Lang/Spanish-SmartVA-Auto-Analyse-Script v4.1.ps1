...
$briefcaseArgumentList = "-jar briefcase.jar -oc -em -id $briefcaseFormID -sd " + '"' + $briefcaseStorageDir + '"' + " -ed " + '"' + $briefcaseExportDir + '"' + " -f $briefcaseConvertedFile"
...
if(!(Test-Path $resultsfile)){
    ...
    "VA_ID," + "Años," + "Sexo," + "CoD1," + "Lh1," + "CoD2," + "Lh2," + "CoD3," + "Lh3," + "Todos_los_síntomas," + "PCoD," + "Fecha ingresada" | Add-Content $resultsfile
}
...
if($config.Aggregate -eq '1'){
    ...
    (new-object -ComObject wscript.shell).Popup("No URL specified for Aggregate server, check the config.txt file has the correct values set for Aggregate and AggregateURL config items",0,"Error!")
        ...
        (new-object -ComObject wscript.shell).Popup("No Username or Password specified for Aggregate server, check the config.txt file has the correct values set for Aggregate, AggregateUser and AggregatePass",0,"Error!")
        ...
        $briefcaseArgumentList += " -url $AggregateUrl"
        $AggregateUser = $config.AggregateUser
        $AggregatePass = $config.AggregatePass
        $briefcaseArgumentList += " -u $AggregateUser"
        $briefcaseArgumentList += " -p $AggregatePass"
    }
}
else{
    $briefcaseArgumentList += " -od " + '"' + "$collectDir" + '"'
...
if($config.Aggregate -ne '1'){
...
#if($localpathfound -ne '0' -OR $config.Aggregate -eq '1'){
    ...
    $ObjForm.Text = "SmartVA Entradas de procesamiento..."
    $ObjForm.Height = 100
    $ObjForm.Width = 500
    ...
    $ObjLabel.Text = "ODK Briefcase Conversión comenzada ... "
    $PB.Value = 20
    $ObjForm.Refresh()
    Start-Sleep -Milliseconds 150
        
    $bproc = Start-Process -FilePath java.exe -ArgumentList "$briefcaseArgumentList" -NoNewWindow -Wait -PassThru -RedirectStandardError $logfiledir\BriefcaseOutput.txt
    $bproc
    
    #Run ODK Briefcase with specified Arguments
    "ODK Briefcase conversion started with the following arguments: $briefcaseArgumentList`r" | Add-Content $logfile
    ...
    if($bproc.ExitCode -eq 1){
        ForEach ($line in (Get-Content $logfiledir\BriefcaseOutput.txt | Where-Object {$_ -like '*SEVERE*'})) {
            $errors += $line
            if($config.Aggregate -eq "1"){
                (new-object -ComObject wscript.shell).Popup("$errors `n`nCheck Aggregate Server Information section in the config.txt file is correct.",0,"Error!")
                ...
                (new-object -ComObject wscript.shell).Popup("$errors `n`nCheck the config.txt file settings for ODK Briefcase Storage location matches script expected location, or that the FormID is correct",0,"Error!")
                "$errors : Check ODK Briefcase Storage locations match script expected location and that the FormID is correct`r" | Add-Content $logfile
                $ObjForm.Close()
                $ObjForm.Dispose()
                exit
        ...
        if($errors -eq $null){
	        ...
	        $ObjForm.Refresh()
            Start-Sleep -Milliseconds 150
            $tproc = Start-Process -FilePath SmartVA-Analyze-cli.exe -ArgumentList "--language spanish --country $tariffCountry --hiv $tariffHIV --malaria $tariffMalaria --hce $tariffHCE --freetext $tariffFreetext --figures $tariffFigures `"$tariffInput`" $tariffOutput" -NoNewWindow -PassThru -RedirectStandardError $logfiledir\TariffOutput.txt 
            $tproc
            ...
            Do {
	            ...
	            $ProcessesFound = Get-Process | ? {$ProcessList -contains $_.Name} | Select-Object -ExpandProperty Name
	                If ($ProcessesFound) {
                        $counter++
                        $ObjLabel.Text = "Tariff Procesamiento de análisis ... $Percentage% "
            ...
            if(!(Get-Content $logfiledir\TariffOutput.txt | Where-Object {$_ -like '*Process Completed*'})){
                ...
                if($config.Aggregate -ne "1"){
                    ...
                    if($archiveFormFile.Fullname -ne $null){
                        ...
                        if(($collectFormFile.LastWriteTime.Date -gt $archiveFormFile.LastWriteTime.Date) -or ($collectFormFile.LastWriteTime.Date -eq $archiveFormFile.LastWriteTime.Date -and $collectFormFile.LastWriteTime.TimeofDay -gt $archiveFormFile.LastWriteTime.TimeofDay)){
                            ...
                            try{
                                ...
                                (new-object -ComObject wscript.shell).Popup("Se ha encontrado un problema al copiar al archivo. Por favor mira $($logfile) para informacion adicional",0,"Error!")                       
                        ...
                        try{
                            ...
                            (new-object -ComObject wscript.shell).Popup("Se ha encontrado un problema al copiar al archivo $($logfile) para informacion adicional",0,"Error!")                       
                            "Unable to complete Archive Form update due to: $_.Exception.  Correct this error and re-run analysis to ensure Archive data is updated BEFORE removing VA's from the Tablet." | Add-Content $logfile
                        }
                    ...
                    if($archiveInstanceFiles -ne $null){
                        ...
                        if($errorcount -gt 0){
                            ...
                            (new-object -ComObject wscript.shell).Popup("Se ha encontrado un problema al copiar al archivo.`n`nReview and correct the error then re-run analysis to ensure Archive data is updated BEFORE removing VA's from the Tablet.`n`nFor error details see $($logfile).",0,"Error!")
                        }
                    }else{
                        try{
                            ForEach($collectInstanceFile in $collectInstanceFiles){
                ...
                if($config.figures -eq "true"){
                    ...
                    $source = $tariffOutput + "\figures"
                    $destination = $collectArchiveDir + "\figures"
                    ...
                    try{
                        ...
                        (new-object -ComObject wscript.shell).Popup("Unable to complete Archive of CSMF files due to: $($_).  Correct this error and re-run analysis to attempt rewrite of CSMF figures.`n`nFor error details see $($logfile).",0,"Error!")
                    }


                }

                 #Lookup file names for all Tariff Predictions and current archive results. Display Tariff Predictions to screen and add to archive results.csv file if not already existing
                ...
                if($tariffpredictions -ne $null){
                    ...
                    ForEach ($prediction in $tariffpredictions){
                        ...
                        ForEach ($VA in $VAs){
                            ...
                            $results += "VA ID:  " + $sid + "`nAños:  " + $age + "`nSexo:  " + $gender + "`n`nCausa de la muerte 1:  " + $cod1 + "`nProbabilidad:  " + $lh1 + "`n`nCausa de la muerte 2:  " + $cod2 + "`nProbabilidad:  " + $lh2 + "`n`nCausa de la muerte 3:  " + $cod3 + "`Probabilidad:  " + $lh3 + "`n`n" + "Todos los síntomas:`n" + $as_repl
                            ...
                            if($archiveResults -ne $null){
                                ...
                                if($x -eq 0){
                                    $PhdUCoD = [Microsoft.VisualBasic.Interaction]::InputBox("Ingrese la causa de muerte del médico para lo siguiente:`n`n$results", "Causa del médico de la muerte", "")
                                    try{
                                        $sid + "," + $age + "," + $gender + "," + $cod1 + "," + $lh1 + "," + $cod2 + "," + $lh2 + "," + $cod3 + "," + $lh3 + "," + $as + "," + $PhdUCoD + "," + $datetime + "," | Add-Content $resultsfile -ErrorAction Stop
                        ...
                        if($archiveResultErrorCount -gt 0){
                            (new-object -ComObject wscript.shell).Popup("Could not write results to Archive file $resultsfile due to: `n`n$archiveErrorMsg`n`n 
Si este archivo está abierto, ciérrelo y repita el análisis para capturar los resultados en el archivoSi este archivo está abierto, ciérrelo y repita el análisis para capturar los resultados en el archivo",0,"Error")
                            "No se pudieron escribir los resultados en el archivo de almacenamiento $resultsfile due to: $archiveErrorMsg" | Add-content $logfile
                            exit
                        }
                        $allresults += $results
                    }
                    if($x -gt 0){
                        (new-object -ComObject wscript.shell).Popup("No hay resultados para mostrar. Todos los resultados ya están en el archivo de resultados del archivo.",0,"SmartVA Resultados")
                    }
                }else{
                    (new-object -ComObject wscript.shell).Popup("No hay resultados para mostrar. Esto es normal si el VA procesado no tenía información sobre la edad, o no se proporcionó el consentimiento.",0,"SmartVA Results")
                }
            }
        } 
    }
