...
if(!(Test-Path $resultsfile)){
    ...
    "VA_ID," + "Cause_of_Death," + "Age," + "Sex," + "Date_Entered" | Add-Content $resultsfile
}
...
if($localpathfound -ne '0' -OR $config.Aggregate -eq '1'){
    ...
    if($bproc.ExitCode -eq 1){
        ...
        if($errors -eq $null){
	        ...
	        $ObjForm.Refresh()
            Start-Sleep -Milliseconds 150
            $tproc = Start-Process -FilePath SmartVA-cli-RC4.exe -ArgumentList "--country $tariffCountry --hiv $tariffHIV --malaria $tariffMalaria --hce $tariffHCE --freetext $tariffFreetext --figures $tariffFigures `"$tariffInput`" $tariffOutput" -NoNewWindow -PassThru -RedirectStandardError $logfiledir\TariffOutput.txt 
            $tproc
            $ProcessList = "SmartVA-cli-RC4"
            Do {
                $Percentage = $PB.Value
            ...
            if(!(Get-Content $logfiledir\TariffOutput.txt | Where-Object {$_ -like '*Process Completed*'})){
                ...
                $tariffpredictions = get-childitem "$tariffOutput" -Recurse | where {$_.name -like '*predictions*'}
                ...
                if($tariffpredictions -ne $null){
                    ...
                    ForEach ($prediction in $tariffpredictions){
                        ...
                        ForEach ($VA in $VAs){
                            ...
                            $results += "VA ID:`t`t" + $sid + "`n" + "Cause of Death:`t" + $cod + "`n" + "Age:`t`t" + $age + "`n" + "Sex:`t`t" + $gender + "`n`n"
                            $sid + "," + $cod + "," + $age + "," + $gender | Add-Content $gridfile 
                        
                            #add to results file
                            
                            if($archiveResults -ne $null){
                                ForEach ($archiveResult in $archiveResults){
                                    if ($sid -eq $archiveResult.VA_ID -and $cod -eq $archiveResult.Cause_of_Death){
                                        $x++
                                    }
                                }
                                if($x -eq 0){
                                    try{
                                        $sid + "," + $cod + "," + $age + "," + $gender + "," + $datetime | Add-Content $resultsfile -ErrorAction Stop
                    ...
                    if($config.OutputDisplay -eq "1"){    
                        if($resultcount -lt 6){
                            #Display Tariff Results to screen in easy to read format
                            (new-object -ComObject wscript.shell).Popup("$allresults",0,"SmartVA Results")
                        }else{
                            Import-Csv -Path $gridfile -Header "VA ID","Cause of Death", Age, Sex | Out-GridView -wait –Title "Results"
                        }
                    }
                }else{
